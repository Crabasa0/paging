...
void swap(pt_entry_t in, pt_entry_t out);
pt_entry_t search();
...
allocate_real_page () {
  ...
  if (real_free_addr > real_size){    //out of space?
    if(!overflowed){//DEBUG: tell me if we have overflowed onto BS
      overflowed = true;
      printf("Boi you deadass outta space.\n");
      //show_entries();
    }
    printf("Preparing to search\n");
    ...
    printf("Freed up a page when i ran out of memory.");
...
move_to_bs(pt_entry_t lpt_entry){
  printf("Going to BS!\n");
	vmsim_addr_t real_addr = GET_PAGE_ADDR(lpt_entry);
	bs_write(real_addr, block_no);
	int addr_remover = 0x3ff;
	lpt_entry &= addr_remover;
	lpt_entry |= (block_no << 10);
	block_no = block_no + 1;
	CLEAR_RESIDENT(lpt_entry);
	printf("hi\n");
...
void
move_to_mm(pt_entry_t lpt_entry, vmsim_addr_t real_addr){
  printf("Back from BS!\n");
...
search(){
  ...
  //uint64_t cur_page_no = 0;//make me static!
  ...
  printf("LPTE Retuned from search: %x", lpte);
  return lpte;
}
...
void show_entries(){
  for (int i = 0; i < ENTRIES_LENGTH; i++){
    printf("Entries[%i]: %x",i, entries[i]);
  }
}
